...
import com.qudiancan.backend.enums.HolderType;
...
public class ShopServiceUtil {
    ...
    public static void checkShopVO(ShopVO shopVO) {
        try {
            HolderType holderType = HolderType.valueOf(shopVO.getHolderType());
            String holderIdentify = shopVO.getHolderIdentify();
            boolean holderIdentifyValidity = (HolderType.COMPANY == holderType && checkSocialCreditCodeValidity(holderIdentify)) ||
                    (HolderType.PERSON == holderType && checkNationalIdentificationNumberValidity(holderIdentify));
        ...
        if (!AccountServiceUtil.checkPhoneValidity(shopVO.getTelephone())) {
