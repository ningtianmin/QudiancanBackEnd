...
<div class="layui-container" style="width: 100%; padding: 10px;">
    <div class="layui-form">
        <div class="layui-form-item" style="display: flex;align-items: center; justify-content: left">
            ...
            <button class="layui-btn layui-btn-radius layui-btn-warm" lay-submit lay-filter="newCategory">新增桌台分类
            </button>
        </div>
    </div>
...
<script>
    layui.use(['table', 'form', 'layer'], function () {
        ...
        table.render({
            elem: '#categories',
            url: '${basePath}/shops/${currentShopId}/branches/${currentBranchId}/table_categories',
            page: false,
            cols: [[
                    ...
                    if (updateName.length >= 2 && updateName.length <= 15) {
                        ...
                        layer.prompt({
                            ...
                            if (reg.test(updatePosition)) {
                                ...
                                $.ajax({
                                    method: 'POST',
                                    url: '${basePath}/shops/${currentShopId}/branches/${currentBranchId}/table_categories/' + data.id,
                                    data: {
                                        name: updateName,
                                ...
                                layer.msg('桌台类目排序为1至4位的整数', {time: 800});
                                return false;
                            }
                        })
                    } else {
                        layer.msg('桌台类目名称两个字符到十五个字符', {time: 800});
                        return false;
                    }
                })
            }
        ...
        form.verify({
            name: function (value) {
                if (!(value.length >= 2 && value.length <= 15)) {
                    return '桌台类目名称两个字符到十五个字符';
                }
            },
            position: [/^[1-9][0-9]{0,3}$/, '桌台类目排序为1至4位的整数']
        });
